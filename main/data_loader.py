...
def _load_samples_mask(csv_name, image_type):
    filename_queue = tf.train.string_input_producer(
        [csv_name])
    ...
    filename_i, filename_j, filename_i_mask, filename_j_mask = tf.decode_csv(
        csv_filename, record_defaults=record_defaults)

    file_contents_i = tf.read_file(filename_i)
    file_contents_j = tf.read_file(filename_j)
    file_masks_i = tf.read_file(filename_i_mask)
    file_masks_j = tf.read_file(filename_j_mask)
    ...
    if image_type == '.jpg':
        ...
        image_decoded_B = tf.image.decode_jpeg(
            file_contents_j, channels=model.IMG_CHANNELS)
        image_decoded_A_Mask = tf.image.decode_bmp(file_masks_i)
        image_decoded_B_Mask = tf.image.decode_bmp(file_masks_j)
    elif image_type == '.png':
        image_decoded_A = tf.image.decode_png(
            file_contents_i, channels=model.IMG_CHANNELS, dtype=tf.uint8)
        image_decoded_B = tf.image.decode_png(
    ...
    return image_decoded_A, image_decoded_B, image_decoded_A_Mask, image_decoded_B_Mask
...
def load_data(dataset_name, image_size_before_crop,
    ...
    image_i, image_j, image_i_mask, image_j_mask = _load_samples_mask(
        csv_name, cyclegan_datasets.DATASET_TO_IMAGETYPE[dataset_name])
    inputs = {
        'image_i': image_i,
        'image_j': image_j,
        'image_i_mask': image_i_mask,
        'image_j_mask': image_j_mask
    }
    ...
    inputs['image_i_mask'] = tf.image.resize_images(
        inputs['image_i_mask'], [image_size_before_crop, image_size_before_crop])
    inputs['image_j_mask'] = tf.image.resize_images(
        inputs['image_j_mask'], [image_size_before_crop, image_size_before_crop])
    ...
    if do_flipping is True:
        ...
        inputs['image_i_mask'] = tf.image.random_flip_left_right(inputs['image_i_mask'], seed=1)
        inputs['image_j_mask'] = tf.image.random_flip_left_right(inputs['image_j_mask'], seed=1)
    ...
    inputs['image_i_mask'] = tf.random_crop(
        inputs['image_i_mask'], [model.IMG_HEIGHT, model.IMG_WIDTH, 1], seed=1)
    inputs['image_j_mask'] = tf.random_crop(
        inputs['image_j_mask'], [model.IMG_HEIGHT, model.IMG_WIDTH, 1], seed=1)

    inputs['image_i'] = tf.subtract(tf.div(inputs['image_i'], 127.5), 1)
    inputs['image_i_mask'] = tf.subtract(tf.div(inputs['image_i_mask'], 127.5), 1)
    inputs['image_j'] = tf.subtract(tf.div(inputs['image_j'], 127.5), 1)
    inputs['image_j_mask'] = tf.subtract(tf.div(inputs['image_j_mask'], 127.5), 1)

    # Batch
    if do_shuffle is True:
        inputs['images_i'], inputs['images_j'], inputs['images_i_mask'], inputs['images_j_mask'] = tf.train.shuffle_batch(
            [inputs['image_i'], inputs['image_j'], inputs['image_i_mask'], inputs['image_j_mask']], 1, 5000, 100, seed=1)
    else:
        inputs['images_i'], inputs['images_j'], inputs['images_i_mask'], inputs['images_j_mask'] = tf.train.batch(
            [inputs['image_i'], inputs['image_j'], inputs['image_i_mask'], inputs['image_j_mask']], 1)

    return inputs

def load_data_test(dataset_name, image_size_before_crop,
              do_shuffle=True, do_flipping=False):
    """
